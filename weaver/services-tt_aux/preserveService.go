...
type preserveService struct {
	weaver.Implements[PreserveService]
...
func (psi *preserveService) Preserve(ctx context.Context, info util.OrderTicketInfo, token string) (string, error) {
	...
	go func() {
		defer wg.Done()
		trip, tripData, err3 = psi.travelService.Get().GetTripAllDetailInfo(ctx, info.TripId, info.From, info.To, info.Date, token)
	}()
	...
	order, err = psi.orderService.Get().CreateNewOrder(ctx, order, token)
	if err != nil {
		return "", err
	}

...
type AdminOrderService interface {
	...
	//AddOrder(ctx context.Context, order model.Order, token string) (model.Order, error)
}
...
/*func (aosi *adminOrderService) GetAllOrders(ctx context.Context, token string) ([]model.Order, error) {
	...
	var err1, err2 error
	var ordersFirstBatch, ordersSecondBatch []model.Order
...
func (aosi *adminOrderService) UpdateOrder(ctx context.Context, order model.Order, token string) (model.Order, error) {
	err := util.Authenticate(token, aosi.roles...)
	if err != nil {
...
/*func (aosi *adminOrderService) AddOrder(ctx context.Context, order model.Order, token string) (model.Order, error) {
	err := util.Authenticate(token, aosi.roles...)
	if err != nil {
		return model.Order{}, err
	}

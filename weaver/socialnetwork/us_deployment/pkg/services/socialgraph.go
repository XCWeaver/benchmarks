...
type SocialGraphService interface {
	GetFollowers(ctx context.Context, reqID int64, userID string) ([]string, error)
	GetFollowees(ctx context.Context, reqID int64, userID string) ([]string, error)
	Follow(ctx context.Context, reqID int64, userID string, followeeID string) error
	Unfollow(ctx context.Context, reqID int64, userID string, followeeID string) error
	...
	InsertUser(ctx context.Context, reqID int64, userID string) error
}
...
func (s *socialGraphService) GetFollowers(ctx context.Context, reqID int64, userID string) ([]string, error) {
	logger := s.Logger(ctx)
	logger.Debug("entering GetFollowers", "req_id", reqID, "user_id", userID)

	var followerIDs []string
	numFollowers, err := s.redisClient.ZCard(ctx, userID+":followers").Result()
	if err != nil {
		logger.Error("error reading number of followers from cache", "msg", err.Error())
	}
	if numFollowers > 0 {
		// followers are cached in redis so we retrieve them
		result, err := s.redisClient.ZRange(ctx, userID+":followers", 0, -1).Result()
		if err != nil {
			logger.Error("error reading followers from cache", "msg", err.Error())
			return nil, err
		}
		for _, r := range result {
			followerIDs = append(followerIDs, r)
		}
		...
		_, err = s.redisClient.Pipelined(ctx, func(pipe redis.Pipeliner) error {
			for _, f := range userInfo.Followers {
				err = pipe.ZAddNX(ctx, userID+":followers", redis.Z{
					Member: f.FollowerID,
...
func (s *socialGraphService) GetFollowees(ctx context.Context, reqID int64, userID string) ([]string, error) {
	logger := s.Logger(ctx)
	logger.Debug("entering GetFollowees", "req_id", reqID, "user_id", userID)

	var followerIDs []string
	var followerInfos []FollowerInfo
	numFollowees, err := s.redisClient.ZCard(ctx, userID+":followees").Result()
	if err != nil {
		logger.Error("error reading number of followees from redis", "msg", err.Error())
	}
	if numFollowees > 0 {
		// followees are cached in redis so we retrieve them
		result, err := s.redisClient.ZRange(ctx, userID+":followees", 0, -1).Result()
		if err != nil {
		...
		_, err = s.redisClient.Pipelined(ctx, func(pipe redis.Pipeliner) error {
			for _, f := range followerInfos {
				err = pipe.ZAddNX(ctx, userID+":followees", redis.Z{
					Member: f.FollowerID,
					Score:  float64(f.Timestamp),
				}).Err()
				if err != nil {
...
func (s *socialGraphService) Follow(ctx context.Context, reqID int64, userID string, followeeID string) error {
	logger := s.Logger(ctx)
	...
	go func() {
		...
		_, redisUpdateErr = s.redisClient.Pipelined(ctx, func(pipe redis.Pipeliner) error {
			// followees
			_, err := pipe.ZCard(ctx, userID+":followees").Result()
			if err == nil {
				pipe.ZAddNX(ctx, userID+":followees", redis.Z{
					Member: followeeID,
					Score:  float64(timestamp.Unix()),
				})
			}
			// followers
			_, err = pipe.ZCard(ctx, followeeID+":followers").Result()
			if err == nil {
				pipe.ZAddNX(ctx, followeeID+":followers", redis.Z{
					Member: userID,
					Score:  float64(timestamp.Unix()),
				})
			}
...
func (s *socialGraphService) Unfollow(ctx context.Context, reqID int64, userID string, followeeID string) error {
	logger := s.Logger(ctx)
	...
	go func() {
		...
		_, err3 = s.redisClient.Pipelined(ctx, func(pipe redis.Pipeliner) error {
			...
			_, err := pipe.ZCard(ctx, userID+":followees").Result()
			if err == nil {
				pipe.ZRem(ctx, userID+":followees", followeeID)
			}
			// followers
			_, err = pipe.ZCard(ctx, followeeID+":followers").Result()
			if err == nil {
				pipe.ZRem(ctx, followeeID+":followers", userID)
			}
			return nil
		})
	}()
...
func (s *socialGraphService) InsertUser(ctx context.Context, reqID int64, userID string) error {
	logger := s.Logger(ctx)

...
type UserService interface {
	...
	UploadCreatorWithUserId(ctx context.Context, reqID int64, userID string, username string, new bool) error
...
func (u *userService) UploadCreatorWithUserId(ctx context.Context, reqID int64, userID string, username string, new bool) error {
	logger := u.Logger(ctx)
	...
	return u.composePostService.Get().UploadCreator(ctx, reqID, creator, new)
}
...
func (u *userService) UploadCreatorWithUsername(ctx context.Context, reqID int64, username string, new bool) error {
	logger := u.Logger(ctx)
	...
	return u.UploadCreatorWithUserId(ctx, reqID, userID, username, new)
}

// GetUserId attempts to read the user id from cache and return it
// If not found, it fetches the user from the db and uploads it to cache
func (u *userService) GetUserId(ctx context.Context, reqID int64, username string) (string, error) {
	logger := u.Logger(ctx)

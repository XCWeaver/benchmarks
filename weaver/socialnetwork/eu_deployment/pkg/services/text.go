...
func (t *textService) UploadText(ctx context.Context, reqID int64, text string, new bool) error {
	logger := t.Logger(ctx)
	...
	go func() {
		...
		shortenUrlErr = t.urlShortenService.Get().UploadUrls(ctx, reqID, url_strings, new)
	}()
	// --
	...
	go func() {
		...
		userMentionErr = t.userMentionService.Get().UploadUserMentions(ctx, reqID, usernames, new)
	}()
	// --
	...
	go func() {
		...
		uploadTextErr = t.composePostService.Get().UploadText(ctx, reqID, updatedText, new)
	}()
	// --

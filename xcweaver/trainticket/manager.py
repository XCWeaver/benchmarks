...
GCP_INSTANCE_WRK2         = "trainticket-wrk2-xcweaver"
GCP_INSTANCE_APP          = "trainticket-app-xcweaver"
#GCP_INSTANCE_DB           = "trainticket-db-xcweaver"
...
def gen_weaver_config_gcp():
  ...
  data = toml.load("deploy/xcweaver/weaver-gcp-template.toml")
...
def local_consistency_window():
  ...
  if response.status_code == 200:
      ...
      if values == None:
        return None
      print(values)  # This will print the list of floats
      print(len(values))
      average = statistics.mean(values)
      print(f"Average: {average}")
...
if __name__ == "__main__":
  ...
  commands = [
    #gcp
     'configure', 'deploy', 'start', 'stop', 'info', 'restart', 'clean', 'tests',
    # datastores
    'storage-run', 'storage-info', 'storage-clean', 'consistency-window',
    # eval

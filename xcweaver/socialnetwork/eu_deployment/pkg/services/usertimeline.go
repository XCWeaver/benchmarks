...
type UserTimelineService interface {
	ReadUserTimeline(ctx context.Context, reqID int64, userID string, start int64, stop int64) ([]model.Post, error)
	WriteUserTimeline(ctx context.Context, reqID int64, postID string, userID string, timestamp int64) error
}
...
func (u *userTimelineService) WriteUserTimeline(ctx context.Context, reqID int64, postID string, userID string, timestamp int64) error {
	logger := u.Logger(ctx)
	...
	return u.redisClient.ZAddNX(ctx, userID, redis.Z{
		Member: postID,
...
func (u *userTimelineService) readCachedTimeline(ctx context.Context, userIDStr string, start int64, stop int64) ([]string, error) {
	logger := u.Logger(ctx)
...
func (u *userTimelineService) ReadUserTimeline(ctx context.Context, reqID int64, userID string, start int64, stop int64) ([]model.Post, error) {
	logger := u.Logger(ctx)
	...
	postIDs, err := u.readCachedTimeline(ctx, userID, start, stop)
	if err != nil {
		return nil, err
	}
	...
	if len(postsToCache) > 0 {
		_, err = u.redisClient.ZAddNX(ctx, userID, postsToCache...).Result()
		if err != nil {

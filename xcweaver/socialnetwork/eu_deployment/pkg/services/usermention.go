...
type UserMentionService interface {
	UploadUserMentions(ctx context.Context, reqID int64, usernames []string, new bool) error
}
...
func (u *userMentionService) UploadUserMentions(ctx context.Context, reqID int64, usernames []string, new bool) error {
	logger := u.Logger(ctx)
	...
	return u.composePost.Get().UploadUserMentions(ctx, reqID, userMentions, new)
}

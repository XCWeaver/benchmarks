...
type UrlShortenService interface {
	UploadUrls(ctx context.Context, reqID int64, urls []string, new bool) error
...
func (u *urlShortenService) UploadUrls(ctx context.Context, reqID int64, urls []string, new bool) error {
	logger := u.Logger(ctx)
	...
	return u.composePostService.Get().UploadUrls(ctx, reqID, targetUrls, new)
}
